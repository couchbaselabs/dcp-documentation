...
This is the storage engine component of Couchbase Server. It arranges a record of each document by update sequence ID, a  monotonically increasing value, in a btree. Each document mutation is a assigned a new sequence number, for existing documents in the store, the old sequence value is deleted. This allows it to quickly scan recent documents and deletions since a particular sequence number, or from zero if wants all changes.
...
The tree nodes immutable data and are ref counted, with each node pointed to by one or more parent snapshots, and the root ref count is incremented by each partition write queue manager. 
...
The failover log tracks a history of failover events so that UPR clients can find a safe sequence number to stream a new snapshot with a guarantee of not losing any changes. A failover event, and new failover ID, is when a master is started and it's unknown if the master and was completely in sync with all mutations.
...
Clients will persists the failover log whenever it completely persists a snapshot, as well as the high sequence it most recently persisted.
...
Barrier cookies are unique IDs generated by clients to 
...
2. The hash table bucket where the item will reside is locked.
...
4. Any deletions persisted are then removed from the hashtable, if the items in the hashtable are still in the deleted state.
...
3. ep-engine then sends the failover log for each partition it owns, the latest failover ID and the seq number the client should stream from, using the rules for determining the start seq for a given failover seq.
4. For each partition it wishes to stream, it sends to the server partition number. expected state of active or replica, and latest failover ID and start seq number.
...
6. For each partition with mutations. The server will first send a partition start message, which includes the partition ID it will send, then stream all changes, in by sequence order. The server may send duplicates for  items, but it will never miss an item.
...
3. If the starting seq number is on disk, not memory, it walks the seq btree.
...
5. It sends each items metadata and body to the client.
6. Once it's finished walking the disk btree, it then walks to the in-memory by seq tree, sending each items metadata and body to the client.
...
3. It determine the starting sequence by scanning the failover IDs that match it's stored failover ID, and it takes the smaller of of it's own snapshot seq or the failover ID.
4. It rolls back it's own changes to the starting seq number, by requesting the latest document for each higher seq in its ring buffer, and processing it. If the document doesn't exist, it deletes the document from it's state.
5. It removes the higher seq item IDs from the ring buffer.
6. It asks the server for a snapshot of changes for the last failover ID using the start seq number, and applies them to the ring buffer and it's state.
